...
/*
* Wordpress Info
Plugin Name: WP Subdomains Revisited
Plugin URI: http://www.lontongcorp.com/2012/03/16/wp-subdomains/
Description: Setup your main categories, pages, and authors as subdomains and give them custom themes without or inside multisite. Originally based on <a href="http://www.biggnuts.com/wordpress-subdomains-plugin/">Subster Rejunevation</a>. The plugin is updated to 0.7.0 version by <a href="http://www.lontongcorp.comn/">lontongcorp</a>
Version: 0.8
Author: Erick (IGITS), Alex (CasualGenius)
Author URI: http://www.lontongcorp.com

* LICENSE

    Copyright 2009  Alex Stansfield <alex@casualgenius.com> & Erick Tampubolon <lontongcorp@gmail.com>
...
define( 'WPS_OPT_REDIRECT_CANONICAL', 'wps_redirectcanonical' );
...
class WpsPlugin {
	...
	function addActions() {
		...
		add_action( 'category_edit_form_fields', 'wps_edit_taxonomy' );
		add_action( 'edited_category', 'wps_save_taxonomy', 10, 2 );
		add_action( 'category_add_form_fields', 'wps_edit_taxonomy' );
		add_action( 'edited_category', 'wps_save_taxonomy', 10, 2 );
		
		add_action( 'do_meta_boxes', 'wps_action_page_meta');
		add_action( 'save_post', 'wps_action_save_postdata' );
...
if ( function_exists('register_activation_hook') ) {
    include_once ('plugin/install.php');
